...
pub struct Betweenness<Vertex> {
    pub edges: HashMap<
        (
            Vertex, /* Primeiro vértice do caminho atual */
            Vertex, /* Vértice Atual */
            Vertex, /* Vértice Posterior */
        ),
        f64,
    >,
}

impl<Vertex> Betweenness<Vertex>
where
    Vertex: Eq + Hash + Clone,
{
    pub fn get_max(&self) -> (Vertex, Vertex) {
        self.calculate()
            .iter()
    ...
    pub fn contains(&self, key: &(Vertex, Vertex, Vertex)) -> bool {
        self.edges.get(key).is_some()
    }

    pub fn insert_edge(&mut self, key: (Vertex, Vertex, Vertex), value: f64) {
        self.edges.insert(key, value);
    }

    pub fn sum_of_bellow_edges(&self, origin_vertex: Vertex, to_vertex: Vertex) -> f64 {
    ...
    fn calculate(&self) -> HashMap<(Vertex, Vertex), f64> {
        let mut betweenness_per_edge: HashMap<(Vertex, Vertex), f64> = HashMap::new();
